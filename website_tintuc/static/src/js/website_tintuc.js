odoo.define('website_tintuc.website_tintuc', function (require) {
'use strict';
...
publicWidget.registry.websiteTintuc = publicWidget.Widget.extend({
    selector: '.website_tintuc',
    events: {
        'click #o_wblog_next_container': '_onNextTintucClick',
     ...
     * @param {Event} ev
     */
    _onNextTintucClick: function (ev) {
        ev.preventDefault();
        var self = this;
    ...
    _onContentAnchorClick: function (ev) {
        ...
        this._forumScrollAction($el, 500, function () {
            window.location.hash = 'tintuc_content';
        });
    },
    /**
     * @private
    ...
    _onShareArticle: function (ev) {
        ...
        var tintucPostTitle = encodeURIComponent($('#o_wblog_post_name').html() || '');
        var articleURL = encodeURIComponent(window.location.href);
        if ($element.hasClass('o_twitter')) {
            var twitterText = core._t("Amazing tintuc article: %s! Check it live: %s");
            var tweetText = _.string.sprintf(twitterText, tintucPostTitle, articleURL);
