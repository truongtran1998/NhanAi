odoo.define('website_tintuc.options', function (require) {
'use strict';
...
if (!$('.website_tintuc').length) {
    return;
}

const NEW_TAG_PREFIX = 'new-tintuc-tag-';
...
options.registry.many2one.include({
    ...
    _selectRecord: function ($opt) {
        ...
        if (this.$target.data('oe-field') === 'author_id') {
            var $nodes = $('[data-oe-model="tintuc.post"][data-oe-id="' + this.$target.data('oe-id') + '"][data-oe-field="author_avatar"]');
            $nodes.each(function () {
...
options.registry.TintucPostTagSelection = options.Class.extend({
    /**
     * @override
     */
    async willStart() {
        ...
        this.tintucPostID = parseInt(this.$target[0].dataset.tintucId);
        ...
        for (const tag of tags) {
            ...
            if (tag['post_ids'].includes(this.tintucPostID)) {
     ...
     */
    _notifyUpdatedTags() {
        this.trigger_up('set_tintuc_post_updated_tags', {
            tintucPostID: this.tintucPostID,
    ...
    async _renderCustomXML(uiFragment) {
        uiFragment.querySelector('we-many2many').dataset.recordId = this.tintucPostID;
    },
});
});
