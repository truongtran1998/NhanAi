...
class WebsiteTintuc(http.Controller):
    ...
    class Home(Home):
        ...
        def index(self, *args, **kw):
            ...
            website_post_ids = request.env['tintuc.post'].search([('is_published', '=', True)], limit=8, order="post_date desc")
            tintuc = requests.get("https://medinet.gov.vn/tin-tuc-su-kien-c1780.aspx")
            thongbao = requests.get("https://medinet.gov.vn/thong-bao-chung-tb1769.aspx")
            vanban = requests.get("https://medinet.gov.vn/van-ban-cua-so-y-te-vb4639.aspx")
            lichcongtac = requests.get("https://medinet.gov.vn/lich-cong-tac-c1019.aspx")
            data_tintuc = self.crawl_tintuc(tintuc)
            data_thongbao = self.crawl_thongbao(thongbao)
            data_vanban = self.crawl_vanban(vanban)
            data_lichcongtac = self.crawl_lichcongtac(lichcongtac)
            return request.render('website.homepage', {
                'website_post_ids': website_post_ids,
                'tintucs': data_tintuc,
                'thongbaos': data_thongbao,
                'vanbans': data_vanban,
                'lichcongtacs': data_lichcongtac,
            })

        def crawl_tintuc(self, tintuc):
            data = []
            i = 0
            soup = BeautifulSoup(tintuc.content, "html.parser")
            content_left = soup.find("div", class_="Content_left")
            storys = content_left.findAll("div", class_="story")
            for story in storys:
                title = story.find("a").text + " - Sở Y Tế HCM"
                print("???????????????????????????")
                print(title)
                print("?/////////////////////////")
                time = story.find("span", class_="time").text
                src = story.find("a").attrs["href"]
                src = "https://medinet.gov.vn" + src
                i = i + 1
                print(time)
                data.append({
                    "title": title,
        ...
        def crawl_thongbao(self, thongbao):
            data = []
            i = 0
            soup = BeautifulSoup(thongbao.content, "html.parser")
            content_left = soup.find("div", class_="Content_left")
            storys = content_left.findAll("div", class_="story")
            for story in storys:
                title = story.find("a").text + " - Sở Y Tế HCM"
        ...
        def crawl_vanban(self, vanban):
            data = []
            i = 0
            soup = BeautifulSoup(vanban.content, "html.parser")
            content_left = soup.find("table", class_="GridViewStyle2")
            storys = content_left.findAll("tr")
            for story in storys:
                if not story.find("a"):
                    continue
                title = story.findAll("a")[1].text + " - Sở Y Tế HCM"
                print("???????????????????????????")
                print(title)
                print("?/////////////////////////")
                src = "https://medinet.gov.vn" + story.find("a").attrs["href"]
                time = story.findAll("td")[-1].text
                print(time)
                i = i + 1
                data.append({
                    "title": title,
        ...
        def crawl_lichcongtac(self, lichcongtac):
            data = []
            i = 0
            soup = BeautifulSoup(lichcongtac.content, "html.parser")
            content_left = soup.find("div", class_="Content_left")
            storys = content_left.findAll("div", class_="story")
            for story in storys:
                title = story.find("a").text + " - Sở Y Tế HCM"
